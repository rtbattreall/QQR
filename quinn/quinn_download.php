...
//create FileManager object for size stuff
...
if($action=="delete"){
    ...
    $fm->delete_file($fileID,$file_name,$user->get_hold_dir($clientID));
...
<TABLE BORDER="0" WIDTH="100%" CELLSPACING="0" CELLPADDING="0">
	...
	<TR>
      ...
      <A CLASS="title">Quinn's Download</A> <BR>  <table border="0" cellspacing="0" cellpadding="3">

    <tr> 

      <td>

		<form name="clientSelect" method="post" action="quinn_download2.php"> 

		<?$user->client_select(true)?></form>
    ...
    <tr> 
      ...
      <td CLASS="small">Recent Client Uploads <br><i>Files listed are from the last 4 days, maximum 20 files listed.</i></td>
	...
	<?while($row=$user->get_recent_results()){?>

		<tr>

			<td CLASS="small"><?echo $row["name"]?></td>

			<td valign="top"><a CLASS="smallnav8" href="<?echo $PHP_SELF?>?clientID=<?echo $row["ID"]?>&action=download&filename=<?echo $row["file_name"]?>&fileID=<?echo $row["fileID"]?>"><?echo $row["file_name"]?></td>

			<td CLASS="smallnav" valign="top"><?echo $row["status"]?></td>
			<td CLASS="smallnav" valign="top"><?echo $row["udate"]?></td>
			<td CLASS="smallnav" valign="top"><?echo $fm->format_size($row["file_size"])?></td>

			<td valign="top"><form method="post" action="">

				<input type="hidden" name="fileID" value="<?echo $row["fileID"]?>">

				<input type="hidden" name="file_name" value="<?echo $row["file_name"]?>">

				<input type="hidden" name="clientID" value="<?echo $row["ID"]?>">
		...
		<tr>
			<td COLSPAN="3" CLASS="small"><i>Comments</i><br><?echo stripslashes($row["comments"])?><BR>&nbsp;</td>
			<td COLSPAN="2"></td>
		</tr>
				</form>
	...
	<TR>
		<TD BGCOLOR="#FFFFFF">&nbsp;<BR><TABLE BORDER="0" CELLPADDING="3" CELLSPACING="0"><TR><TD><form><input type="button" value="Add/Edit Users" onClick="javascript:location.href='quinn_edit_users.php'"></form></TD><TD><form method="post" action="loginps.php"><input type="hidden" name="action" value="logout"><input type="submit" ALT="" WIDTH="56" HEIGHT="21" BORDER="0" VSPACE="3" VALUE="Log Out"></form></TD></TR></TABLE><BR>&nbsp;</TD>
	</TR>
